...
Before MCP, every AI integration was custom code. MCP standardises that interface so any compatible AI client (Claude, GPT-4o, etc.) can discover and call any MCP tool server automatically.
...
| **MCP Client** | The AI model that discovers and calls tools | GPT-4o via Azure OpenAI function calling |
...
> 💡 East US has the best GPT-4o availability. If you are in India, also try `eastus2` or `swedencentral`.
...
public static class AzureTools
{
    ...
    public static string CalculateTokenCost(
        ...
        [Description("Model name: gpt-4o or gpt-4o-mini")] string model = "gpt-4o")
    {
        ...
        Tip: Switch to gpt-4o-mini for ~15x cost reduction on the same task.
        """;
    }
...
The AI Agent accepts chat messages, connects to GPT-4o on Azure OpenAI, and calls your MCP Server tools when needed.
...
4. Deploy **gpt-4o** → Deployment name: `gpt-4o-demo` | TPM: `10K`
5. Deploy **gpt-4o-mini** → Deployment name: `gpt-4o-mini-demo` | TPM: `10K`
...
az webapp config appsettings set \
  ...
  AzureOpenAI__DeploymentName="gpt-4o-demo" \
...
curl -X POST https://apim-mcp-demo.azure-api.net/agent/api/chat \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: TENANT_A_KEY" \
  -d '{"message": "I used 1000 prompt tokens and 500 completion tokens with gpt-4o. How much did that cost?"}'
