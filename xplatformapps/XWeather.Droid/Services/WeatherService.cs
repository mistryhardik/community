...
using XWeather.Droid.Models;

namespace XWeather.Droid.Services
{
        ...
        public static async Task<WeatherResponse> GetWeather(string cityName)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    Log.Info("Weather Service", "Result: " + result);
                    return result;
                }

                Log.Warn("Weather Service", "Response failed with statuscode: " + response.StatusCode);
                return null;
            }
            catch (Exception ex)
            {
                Log.Error("Weather Service", ex.Message);
                return null;
            }
        }
